...
<ul>
  <li>
    This function removes the user's phone number by setting it to an empty
    string.
  </li>
  <li>
    If a userId is provided, it removes the phone number for that specific user;
...
<p>
  ...
  <a href="https://github.com/tkhq/sdk/blob/main/packages/core/src/__clients__/core.ts#L2635">
    core.ts:2635
  </a>
</p>
...
<ParamField body="params" type="object" required={false} path="params">
  <Expandable title="params details">
    ...
    <NestedParam
      ...
      required={false}
    >
      user ID to remove a specific user's phone number (defaults to the current
      session's userId).
    </NestedParam>
  </Expandable>
</ParamField>
...
<ResponseField name="returns" type="string" required={true}>
  A promise that resolves to the userId of the user whose phone number was
  removed.
</ResponseField>

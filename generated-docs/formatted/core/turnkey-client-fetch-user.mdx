...
<ul>
  <li>
    Retrieves user details from the Turnkey API using the provided userId and
    organizationId, or defaults to those from the active session.
  </li>
  <li>
    If no userId is provided, the userId from the current session is used.
  </li>
  <li>
    If no organizationId is provided, the organizationId from the current
    session is used.
  </li>
  <li>
...
<p>
  ...
  <a href="https://github.com/tkhq/sdk/blob/main/packages/core/src/__clients__/core.ts#L2394">
    core.ts:2394
  </a>
</p>
...
<ParamField body="params" type="object" required={false} path="params">
  <Expandable title="params details">
    <NestedParam
      ...
      required={false}
    >
      organization ID to specify the sub-organization (defaults to the current
    ...
    <NestedParam
      ...
      required={false}
    >
      user ID to fetch specific user details (defaults to the current session's
      userId).
    </NestedParam>
  </Expandable>
</ParamField>

...
<ul>
  <li>
    This function removes the user's email address by setting it to an empty
    string.
  </li>
  <li>
    If a userId is provided, it removes the email for that specific user;
...
<p>
  ...
  <a href="https://github.com/tkhq/sdk/blob/main/packages/core/src/__clients__/core.ts#L2526">
    core.ts:2526
  </a>
</p>
...
<ParamField body="params" type="object" required={false} path="params">
  <Expandable title="params details">
    ...
    <NestedParam
      ...
      required={false}
    >
      user ID to remove a specific user's email address (defaults to the current
      session's userId).
    </NestedParam>
  </Expandable>
</ParamField>

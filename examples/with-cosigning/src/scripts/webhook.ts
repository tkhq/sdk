...
async function main() {
  ...
  if (command === "-register") {
    const webhookName =
      ...
      `cosigning-activity-webhook-${Date.now()}`;
    ...
    console.log(
      [
        ...
        "",
        "All ACTIVITY_UPDATES events (including sub-org activities) will now",
        "be delivered to your webhook. When an activity reaches CONSENSUS_NEEDED,",
        "the webhook logs the cosign command — run it in a second terminal to approve.",
      ].join("\n"),
    );
  } else {

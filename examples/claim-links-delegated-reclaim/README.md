...
Locally the reclaim fires automatically via `setTimeout` after the TTL expires. In production, replace with a scheduled cron job.

## Note on storage

Claim state and sweep keys are stored in-memory for this demo and reset on server restart. In production, persist them to a database (with the sweep key encrypted at rest) so they survive restarts and can be used by a scheduled cron job.

...
async function main() {
  ...
  if (values.length === 0) throw new Error("usage: pnpm preassociate -- handle:numeric_id [...] [--reallocate]");
  ...
  for (const target of targets) {
    ...
    for (const organizationId of candidates.organizationIds) {
      ...
      const isClaimed = (allocation.users ?? []).some((user) =>
        user.oauthProviders.some(
          (p) => p.subject === target.numericId || p.subject === `x:${target.numericId}`,
        ),
      );
    ...
    if (unclaimed) {
      console.log(`SKIP @${target.handle} ${target.numericId}: ${unclaimed} (unclaimed allocation exists)`);
      continue;
    }
    if (claimed && !reallocate) {
      console.log(`CLAIMED @${target.handle} ${target.numericId}: ${claimed} (pass --reallocate to allocate again)`);
      continue;
    }

...
assert.equal(
  assertClaimMatches(token("16088008"), "allocation:claim:x:16088008:@turnkey"),
  "16088008",
);
...
assert.equal(
  assertClaimMatches(
    token("x:16088008"),
    "allocation:claim:x:16088008:@turnkey",
  ),
  "16088008",
);
...
assert.throws(
  ...
  (error: unknown) =>
    error instanceof ClaimGateError && error.message === CLAIM_MISMATCH_MESSAGE,
);
assert.throws(
  () =>
    assertClaimMatches(token("@mutable_handle"), "allocation:claim:x:16088008"),
  ClaimGateError,
);

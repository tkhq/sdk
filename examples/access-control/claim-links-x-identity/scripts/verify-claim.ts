...
async function main() {
  ...
  if (
    org.rootQuorum?.threshold !== 1 ||
    org.rootQuorum.userIds.length !== 1 ||
  ...
  for (const policy of org.policies ?? []) {
    if (policy.policyName === "Deny allocation backend signing after handoff")
      backendDeny = true;
    if (policy.policyName === "Allow the bound X claimant to sign")
      claimantAllow = true;
    if (
      policy.policyName === "Allow allocation backend to mint claimant sessions"
    )
      backendLogin = true;
  }
  if (!backendDeny || !claimantAllow)
    throw new Error("required signing policies are missing");
  if (!backendLogin) {
    console.warn(
      "note: no oauth_login allow for the backend; repeat logins through this app will be denied",
    );
  }

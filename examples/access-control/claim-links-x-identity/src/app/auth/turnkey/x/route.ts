...
async function step<T>(name: string, fn: () => Promise<T>): Promise<T> {
  try {
    ...
    console.error(
      `claim step FAILED: ${name}: ${e instanceof Error ? e.message : String(e)}`,
    );
    throw e;
  }
}
...
export async function POST(req: Request) {
  ...
  if (typeof body.auth_code !== "string")
  ...
  if (!subOrgId)
    return NextResponse.json(
      { error: "Missing claim allocation" },
      { status: 400 },
    );
  ...
  try {
    ...
    const authenticated = await step("oauth2Authenticate", () =>
      parent.oauth2Authenticate({
    ...
    const numericXId = assertClaimMatches(
    ...
    if (!claimantExists) {
      ...
      const created = await step("createUsers", () =>
        subOrg.createUsers({
          users: [
            {
              ...
              authenticators: [],
              oauthProviders: [
                { providerName: "X", oidcToken: authenticated.oidcToken },
              ],
              userTags: [],
            },
          ],
      ...
      if (!claimantUserId)
        throw new Error("createUsers returned no claimant user ID");
      console.info(`claim: created claimant ${claimantUserId}`);
      await step("createPolicy(claimant allow)", () =>
        subOrg.createPolicy(claimantSignAllowPolicy(claimantUserId)),
      );
      await step("updateRootQuorum", () =>
        subOrg.updateRootQuorum({ threshold: 1, userIds: [claimantUserId] }),
      );
    }

    // oauth_login both proves the attached provider and creates the claimant's session.
    const login = await step("oauthLogin", () =>
      parent.oauthLogin({
    ...
    if (error instanceof ClaimGateError) {
      ...
      return NextResponse.json(
        { error: CLAIM_MISMATCH_MESSAGE },
        { status: 403 },
      );
    }
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      { error: `Error performing OAuth 2.0 authentication: ${message}` },
      { status: 400 },
    );
  }
}

...
import { useTurnkey, AuthState, WalletSource } from "@turnkey/react-wallet-kit";
...
export default function Dashboard() {
  ...
  const embeddedWallets = useMemo(
    () => (wallets ?? []).filter((w) => w.source === WalletSource.Embedded),
    [wallets],
  );

  // Group embedded-wallet account addresses by their address format.
  const addressesByFormat = useMemo(() => {
    const map: Record<string, string[]> = {};
    for (const w of embeddedWallets) {
      for (const a of w.accounts) {
        (map[a.addressFormat] ??= []).push(a.address);
      }
    }
    return map;
  }, [embeddedWallets]);

  const evmAddresses = addressesByFormat["ADDRESS_FORMAT_ETHEREUM"] ?? [];
  const solAddresses = addressesByFormat["ADDRESS_FORMAT_SOLANA"] ?? [];

  const evmAddress = evmAddresses[0];
  const solAddress = solAddresses[0];

  const allAddresses = [...evmAddresses, ...solAddresses];
  const signAddress =
  ...
  const onSignMessage = async () => {
    try {
      ...
      // signRawPayload returns r/s/v at the top level. ed25519 (Solana) has no
      // recovery id, so its signature is r + s; ecdsa is r + s + v.
      const { r, s, v } = res;
      if (r && s) {
        setSignature(isEd25519 ? `0x${r}${s}` : `0x${r}${s}${v}`);
      } else {
        setSignature("(no signature returned)");
      }
    } catch (e) {
      console.error(e);
      setSignErr(e instanceof Error ? e.message : "Failed to sign message.");
    } finally {
      setSigning(false);
    }
  };
  ...
  const onSendEth = async () => {
    try {
      ...
      console.error(e);
      setEthErr(e instanceof Error ? e.message : "Failed to send ETH.");
    } finally {
      setEthSending(false);
    }
  };
  ...
  const onSendSol = async () => {
    try {
      ...
      console.error(e);
      setSolErr(e instanceof Error ? e.message : "Failed to send SOL.");
    } finally {
      setSolSending(false);
    }
  };

...
export async function estimateFees(feeParams: {
    ...
    return feePerByte * (INPUT_BYTE_SIZE_UPPER_BOUND*numInputs + OUTPUT_BYTE_SIZE_UPPER_BOUND*numOutputs)
}
...
async function getFeePerByte(network: Network) {
    try {
      const url = network.bech32 === 'bc' ? "https://mempool.space/api/v1/fees/recommended" : "https://mempool.space/testnet/api/v1/fees/recommended";

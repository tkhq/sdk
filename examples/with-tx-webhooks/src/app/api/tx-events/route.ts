...
function toSseChunk(payload: TxStatusWebhookSseMessage) {
  const data = JSON.stringify(payload);
  return new TextEncoder().encode(`data: ${data}\n\n`);
}

export function GET(request: Request) {
  const store = getTxStatusWebhookEventStore();

  const stream = new ReadableStream({
    start(controller) {
      let isClosed = false;

      const send = (payload: TxStatusWebhookSseMessage) => {
        if (isClosed) {
          return;
        }
      ...
      const handleWebhookEvent = (event: TxStatusWebhookEventEnvelope) => {
        send({
          type: "webhook",
          event,
        });

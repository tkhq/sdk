...
async function main() {
  ...
  const apiPrivateKey = requireEnv("API_PRIVATE_KEY");
  const apiPublicKey = requireEnv("API_PUBLIC_KEY");
  const organizationId = requireEnv("ORGANIZATION_ID");
  const walletName = env("WALLET_NAME", "Spark Wallet");
  ...
  const identityAccount = wallets.accounts.find(
    ({ address }) => address === sparkAddress,
  );
  const depositAccount = wallets.accounts.find(
    ({ path, addressFormat }) =>
      path === depositPath && addressFormat === "ADDRESS_FORMAT_COMPRESSED",
  );
  if (!identityAccount) {

...
(
    async () => {
        ...
        if (args.length < 2) {
            console.log("Invalid client secret provided. To input a client secret run: pnpm run credential-upload -- <client_secret>");
            process.exit(1);
        }

        if (!process.env.X_CLIENT_ID) {
            console.log("Invalid client ID provided. Please provide it in your .env.local file");
            process.exit(1);
        }
        ...
        try {
            ...
            const createOauth2CredentialResponse = await turnkeyClient.apiClient().createOauth2Credential({
            ...
            console.log("Enter this ID in your .env.local for the OAUTH2_CREDENTIAL_ID environment variable");
            console.log("OAuth 2.0 Credential ID: " + createOauth2CredentialResponse.oauth2CredentialId);
        } catch (e) {

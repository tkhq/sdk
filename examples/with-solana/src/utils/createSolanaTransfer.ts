...
/**
 * Creates a versioned Solana transfer and signs it with Turnkey.
 * @param signer
 * @param fromAddress
 * @param toAddress
 * @param amount amount to send in LAMPORTS (one SOL = 1000000000 LAMPS)
 */
export async function createAndSignVersionedTransfer(input: {
  signer: TurnkeySigner;
  fromAddress: string;
  toAddress: string;
  amount: number;
...
export async function signVersionedTransfers(input: {
  signer: TurnkeySigner;
  fromAddress: string;
  unsignedTxs: VersionedTransaction[];

...
| [`privy-migration`](/examples/key-management/privy-migration/)                                     | Server-side Node example that migrates a Privy embedded wallet private key into a Turnkey-managed key over shared HPKE. Supports EVM + Solana and all three Privy ownership models (app-owned = unattended bulk; user-owned = login-triggered; 2-of-2 quorum). Includes a mock mode. |

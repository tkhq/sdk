 ...
 */
export const selectInputUTXOs = <TValue extends number | bigint = number | bigint, TUTXO extends UTXOLike<TValue> = UTXOLike<TValue>>(inputs: TUTXO[], outputValue: NoInfer<TValue>): [outputs: TUTXO[], change: bigint, unused: TUTXO[]] => {
    if (inputs.length === 0) {
    ...
    if (remainingOutputValue > 0n) {
        throw new TurnkeyError(`Insufficient input value for selectInputUTXOs. Need ${remainingOutputValue} more.`);
    }
 ...
 */
export const smallestUTXOValueFirst = <TValue extends number | bigint = number | bigint, TUTXO extends UTXOLike<TValue> = UTXOLike<TValue>>(a: TUTXO, b: TUTXO): number => {
 ...
 */
export const largestUTXOValueFirst = <TValue extends number | bigint = number | bigint, TUTXO extends UTXOLike<TValue> = UTXOLike<TValue>>(a: TUTXO, b: TUTXO): number => {
    return BigInt(a.value) > BigInt(b.value) ? -1 : 1;
}
...
import { TurnkeyLogo } from "../design/Svg";
...
export function VerifyPage(props: VerifyPageProps) {
  ...
  useEffect(() => {
    ...
    const runAction = async () => {
      try {
        await new Promise((resolve) => setTimeout(resolve, 200)); // slight delay to allow modal animation to complete. The verifyAppProofs call can sometimes freeze the UI while it runs
  ...
  return (
    ...
    >
      {!hasCompleted && (
        <div className="size-full absolute text-center text-lg font-semibold flex items-center justify-center  animate-pulse">
          <span>Verifying App Proofs</span>
        </div>
      )}
      ...
      >
        <div className="flex flex-row items-center justify-center">
          ...
          >
            <TurnkeyLogo className="w-auto h-[18px]" />
          </a>
        </div>
        {isTextPinging && (
          <div className="flex flex-row items-center justify-center absolute animate-ping text-icon-text-light dark:text-icon-text-dark">
            <span>Secured by</span>
            <a
              href="https://turnkey.com"
              target="_blank"

...
export async function verify(appProof: v1AppProof, bootProof: v1BootProof): Promise<void> {
  // 1. Verify App Proof
...
export function verifyAppProofSignature(appProof: v1AppProof): void {
  ...
  if (publicKeyBytes.length !== 130) {
    throw new Error(`Expected 130 bytes (encryption + signing keys), got ${publicKeyBytes.length} bytes`);
  }
...
export async function verifyCertificateChain(cabundle: Uint8Array[], rootCertPem: string, leafCert: Uint8Array, timestampMs: number): Promise<void> {
  try {
    ...
    const rootSha = createHash("sha256").update(rootDer).digest("hex").toUpperCase();
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (i === path.length - 1) { // is root
        ...
        const ok = await cert.verify( { publicKey: cert.publicKey, date: appProofDate });
...
export function verifyCoseSign1Sig(coseSign1: any, leaf: Uint8Array): void {
  ...
  const tbs = Buffer.from(
    CBOR.encode(["Signature1", Buffer.from(protectedHeaders), new Uint8Array(0), Buffer.from(payload)])
  );
  ...
  verify.update(tbs);
  verify.end();
  const ok = verify.verify({ key: pubKey, dsaEncoding: "ieee-p1363" as any }, Buffer.from(signature));

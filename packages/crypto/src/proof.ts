...
let crypto: Crypto | PeculiarCrypto;
...
if (
  ...
  // Use native WebCrypto (browser / modern Node). This is way faster than Peculiar's polyfill in web environments
  crypto = globalThis.crypto as Crypto;
  x509.cryptoProvider.set(globalThis.crypto as Crypto);
} else {
  // Fallback for React Native / Node environments without subtle
  const peculiar = new PeculiarCrypto();
  crypto = peculiar;
  x509.cryptoProvider.set(peculiar);
}

...
export class TurnkeyBrowserClient extends TurnkeySDKClientBase {
  ...
  /**
   * Creates a read-only session. This method infers the current user's organization ID. To be used in conjunction with `currentUserSession`, which results in a TurnkeyBrowserClient with an attached session header that enables read requests.
   *
   * @returns {Promise<SdkApiTypes.TCreateReadOnlySessionResponse>}
   */

    ...
    if #available(iOS 15.0, *) {
      ...
      self.passKeyDelegate = PasskeyDelegate { error, result in
        ...
        if let assertionResult = result?.assertionResult {
          // Return a NSDictionary instance with the received authorization data
          let authResponse: NSDictionary = [
            "rawAuthenticatorData": assertionResult.rawAuthenticatorData.base64URLEncodedString(),
            "rawClientDataJSON": assertionResult.rawClientDataJSON.base64URLEncodedString(),
            "signature": assertionResult.signature.base64URLEncodedString(),
          ];
            
            print("valid signature1: ",assertionResult.signature);

            print("valid signature: ",assertionResult.signature.base64URLEncodedString());

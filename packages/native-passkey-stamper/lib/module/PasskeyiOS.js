...
export class PasskeyiOS {
  ...
  static async register(request) {
    ...
    try {
      const response = await NativePasskey.register(rpId, challenge, name, userID, withSecurityKey);
  ...
  static async authenticate(request) {
    let withSecurityKey = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    try {
      const response = await NativePasskey.authenticate(request.rpId, request.challenge, withSecurityKey);

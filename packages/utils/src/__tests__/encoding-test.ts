import { base64urlEncode, base64urlDecode, base58checkDecode } from '../encoding.ts';
...
describe('Encoding Utilities', () => {
    ...
    describe('Base58 Check', () => {
        test('base58checkDecode should correctly decode a valid Bitcoin address', () => {
            ...
            const expectedPublicKeyHash = Array.from(new Uint8Array([0, 118, 128, 173, 236, 142,
                171, 202, 186, 198, 118, 190,
                158, 131, 133,  74, 222,  11,
                210,  44, 219]));
            const decoded = base58checkDecode(bitcoinAddress);
            const decodedArray = Array.from(decoded);
            expect(decodedArray).toEqual(expect.arrayContaining(expectedPublicKeyHash));
        });
        ...
        test('base58checkDecode should throw on invalid checksum', () => {
            ...
            expect(() => base58checkDecode(invalidBitcoinAddress)).toThrow('Invalid checksum');
        });
        ...
        test('base58checkDecode should throw on invalid character', () => {
            ...
            expect(() => base58checkDecode(addressWithInvalidChar)).toThrow('Invalid character found');
        });
    });
});

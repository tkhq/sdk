...
describe("TurnkeySparkWallet", () => {
    ...
    beforeAll(async () => {
        ...
        const pendingTransfers = await sparkClient.getPendingTransfers(transferAddress)
...
class TurnekySparkWalletTest extends TurnkeySparkWallet {
    async __claimTransfer(transfer: Transfer) {
        return (this as unknown as WalletWithClaimTransfer).claimTransfer({ transfer, emit: true })
    }

    async __claimTransferBatch(transfers: Transfer[]) {
        return (this as unknown as WalletWithClaimTransfer).claimTransferBatch(transfers, true)
    }
}

...
describe("TurnkeyAccount", () => {
  ...
  testCase("returns correct address", async () => {
    expect(walletClient.account.address).toEqual(expectedEthAddress);
    expect(walletClient.getAddresses()).toEqual([expectedEthAddress]);
  });

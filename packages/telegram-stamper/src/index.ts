...
export class TelegramStamper {
  ...
  constructor(config?: TTelegramStamperConfig) {
    ...
    if (window.Telegram?.WebApp == null) {
      throw new TelegramStamperError("Cannot initialize telegram stamper in non telegram mini-app environment");
    }
  ...
  async init() {
    if (this.stamper) {
      ...
      // insert public key
      await window.Telegram.WebApp.CloudStorage.setItem(PUBLIC_KEY_CLOUD_STORAGE_KEY, this.stamper.apiPublicKey, (err: any, stored: boolean) => {
          if (err != null || !stored) {
            throw new TelegramStamperError(`Failed inserting api public key into Telegram Cloud Storage${err && `: ${err}`}`)
          }
      })

      // insert private key
      await window.Telegram.WebApp.CloudStorage.setItem(PRIVATE_KEY_CLOUD_STORAGE_KEY, this.stamper.apiPrivateKey, (err: any, stored: boolean) => {
        if (err != null || !stored) {
          // remove public key from storage if private key could not be stored
          try {
            this.clearKey(PUBLIC_KEY_CLOUD_STORAGE_KEY)
          } catch (err) {
            console.log(`Failed removing public key from Telegram Cloud Storage stored at: ${PUBLIC_KEY_CLOUD_STORAGE_KEY}`)
          }
          throw new TelegramStamperError(`Failed inserting api private key into Telegram Cloud Storage${err && `: ${err}`}`)
        }
      })
    } else {
      ...
      // get public key
      await window.Telegram.WebApp.CloudStorage.getItem(PUBLIC_KEY_CLOUD_STORAGE_KEY, (err: any, pubKey: string) => {
        if (err != null || !pubKey) {
          throw new TelegramStamperError(`Failed getting api public key from Telegram Cloud Storage${err && `: ${err}`}`)
        }
      ...
      // get private key
      await window.Telegram.WebApp.CloudStorage.getItem(PUBLIC_KEY_CLOUD_STORAGE_KEY, (err: any, privKey: string) => {
        if (err != null || !privKey) {
          throw new TelegramStamperError(`Failed getting api public key from Telegram Cloud Storage${err && `: ${err}`}`)
        }
  ...
  async stamp(payload: string) {
    if (!this.stamper) {
      throw new TelegramStamperError("Cannot stamp with unintialized telegram stamper, call TelegramStamper.init()")
    }
  ...
  async clearKey(key: string) {
    await window.Telegram.WebApp.CloudStorage.removeItem(key, (err: any, removed: boolean) => {
      if (err || !removed) {
        throw new TelegramStamperError(`Failed removing key: ${key}${err && `: ${err}`}`)
      }
    })
  }
}

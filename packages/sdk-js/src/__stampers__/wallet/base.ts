...
type ContextMap = Partial<Record<Chain, WalletContext>>;
...
export class CrossPlatformWalletStamper implements TStamper {
  ...
  async init(): Promise<void> {
    ...
    const add = (chain: Chain, wallet: WalletInterface) => {
      this.ctx[chain] = {
        wallet,
  ...
  setProvider(chain: Chain, provider: WalletRpcProvider): void {
    this.getCtx(chain).provider = provider;
    this.activeChain = chain;
  }

  getWalletInterface(chain: Chain = this.defaultChain()): WalletInterface {
  ...
  private getCtx(chain: Chain): WalletContext {

...
export class TurnkeySDKClientBase {
  ...
  for (const endpointPath in swaggerSpec.paths) {
    ...
    return this.request("${endpointPath}", {
      ...
      const resultKey = operationNameWithoutNamespace + "Result";
      const versionedMethodName = latestVersions[resultKey].formattedKeyName;
      console.log(`Versioned method name: ${versionedMethodName}`);
      codeBuffer.push(
    ...
    if (operationNameWithoutNamespace === "SignTransaction") {
      codeBuffer.push(
        `\n\tstamp${operationNameWithoutNamespace} = async (input: SdkApiTypes.${inputType}): Promise<TSignedRequest | undefined> => {
      if (!this.stamper) {
        return undefined;
      }
      console.time("stampSignTransaction");
      const fullUrl = this.config.apiBaseUrl + "${endpointPath}";
      console.time("stampSignTransactionJsonify");
      const body = JSON.stringify(input);
      console.timeEnd("stampSignTransactionJsonify");
      const stamp = await this.stamper.stamp(body);
      console.timeEnd("stampSignTransaction");
      return {
        body: body,
        stamp: stamp,
        url: fullUrl,

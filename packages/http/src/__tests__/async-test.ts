...
describe("Test createActivityPoller method", () => {
  ...
  const sampleCreatePrivateKeysInput: TCreatePrivateKeysBody = {
  ...
  test("`createActivityPoller` should return data after activity completion", async () => {
    ...
    const result = await activityPoller(sampleCreatePrivateKeysInput);
  ...
  test("`createActivityPoller` should throw a rich error when activity requires consensus", async () => {
    const activityPoller = createActivityPoller({
      client: turnkeyClient,
      requestFn: turnkeyClient.createPrivateKeys,
    });
    ...
    try {
      await activityPoller(sampleCreatePrivateKeysInput);
  ...
  test("`createActivityPoller` should throw a rich error when activity is rejected", async () => {
    const activityPoller = createActivityPoller({
      client: turnkeyClient,
      requestFn: turnkeyClient.createPrivateKeys,
    });
  ...
  test("`createActivityPoller` should throw a rich error when activity fails", async () => {
    const activityPoller = createActivityPoller({
      client: turnkeyClient,
      requestFn: turnkeyClient.createPrivateKeys,
    });
  ...
  test("`createActivityPoller` should also work with synchronous activity endpoints", async () => {
    const activityPoller = createActivityPoller({
      client: turnkeyClient,
    ...
    const result = await activityPoller({
...
describe("Test deprecated withAsyncPolling method", () => {
  beforeEach(async () => {
    jest.resetAllMocks();
    const { privateKey, publicKey } = await readFixture();
